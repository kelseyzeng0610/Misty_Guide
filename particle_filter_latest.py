...
class ParticleFilter:
                    ...
                    if expected_image is not None:
                        ...
                        if i == 0:  # Only show for first particle
                            ...
                            plt.imshow(cv2.cvtColor(
                                expected_image, cv2.COLOR_BGR2RGB))
                            plt.title(f"Expected Image - {orientation}")
                            plt.show()

                        similarity = self.image_similarity(
    ...
    def estimate_position(self):
        ...
        x_estimate = np.average(
            self.particles[:, 0], weights=self.particle_weights)
        y_estimate = np.average(
            self.particles[:, 1], weights=self.particle_weights)
...
class RobotLocalizer:
    ...
    def extract_movement(self, eventName, data):
        ...
        # Assuming robot moves in the direction of heading
                    ...
                    if not data_queue.empty():
                        ...
                        if event_type == "DriveEncoders":
                            ...
                            if actual_image is not None:
                                ...
                                step += 1
                                logging.info(
                                    f"Step: {step}, Estimated Position: {estimated_position}")
                            else:
                                logging.warning(
                                    "Actual image is None. Skipping update.")

                        elif event_type == "IMU":
                            logging.debug(
                                "Received IMU event. Currently not used.")
...
def handle_sensor_data(event_type, message):
    ...
    logging.debug(
        f"Handling sensor data: {event_type}, event_data: {event_data}")
